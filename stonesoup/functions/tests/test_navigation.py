...
from stonesoup.functions.navigation import earthSpeedFlatSq, earthSpeedSq, earthTurnRateVector, \
    ...
    getEulerCorrection, euler2rotationVector
...
def test_functions_using_states():
    ...
    euler_angles = (np.array([-14.03624347,  -2.7770768, 0.]),
                    np.array([3.37033997, 38.91607362, 0.]))
    ...
    state_test_new = deepcopy(state_test)
    state_test_new[vang_idx] = euler_angles[1].reshape(-1, 1) - state_test[vang_idx]

    assert np.allclose(getEulerCorrection(k, state_test),
                       state_test_new)

    assert np.allclose(getEulerCorrection(0, state_test),
                       state_test)
