...
def getForceVector(state, latLonAlt0):
    ...
    for ipoint in range(npts):
        # get the 3D local position, acceleration
        # velocity
        p_n = localpos[:, ipoint]
...
def getEulerCorrection(K: float, state):
    """Update the Euler angles motion due
        to the Ornstein-Uhlenbeck coefficient in
        the direction of travel.
        As it is now it is hard-coded the 3D indexes
        of velocity and acceleration.

    Parameters
    ----------
    K: float
        Ornstein-Uhlenbeck coefficient

    state: :class:`~.State'
        array containing the 3D target state, dynamics up to acceleration and
        Euler angles and their time derivative
    Return
    ------
    State
        target state updated with the velocities of the
        angles.
    """

    # extract the current Euler angles
    _, dpsiThetaphi_deg = getEulersAngles(state[[1, 4, 7]],
                                          state[[2, 5, 8]])

    # update the states
    state[[10]] = K*(dpsiThetaphi_deg[0] - state[[10]])
    state[[12]] = K*(dpsiThetaphi_deg[1] - state[[12]])
    state[[14]] = K*(dpsiThetaphi_deg[2] - state[[14]])
