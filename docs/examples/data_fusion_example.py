...
# of two separate filters, an extended Kalman and Particle filters.
#
...
# two :class:`~.RadarBearingRange` class radars collecting the measurements
# of the targets, in the same cartesian space. The first radar
# is placed onto a :class:`~.FixedPlatform` while the
# second is on a :class:`~.MovingPlatform`
# The targets follow a straight line trajectory for simplicity.
# For the targets we instantiate the origins and we a transition model
...
# insert some noise for easier prediction in particle
# filters
...
radar_noise = CovarianceMatrix(np.diag([np.deg2rad(0.005), 0.05]))
...
# particle filter using the same specifications.
...
plotter = Plotterly()
plotter.plot_measurements(s1_detections, [0,2]) # maybe consider giving different colors?
